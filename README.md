...
| `volatile` | Default. `ultra` plus the vault: anything over 500 B that no compressor claimed goes to a file and comes back as its two ends and a path. **-76%** on a real corpus. |
...
The case is the shape of the corpus, not the shape of an output: over 8901 real results the largest 10% of calls hold **50.1%** of all the bytes, and the median result is 1261 B. A compressor works the part of the distribution where there is nothing to win. Measured end to end on the corpus, `full` and `ultra` land at **-6%** and `volatile` at **-76%** — 10.66 MB down to 2.56 MB. Weighted by the turns each result would still be re-read for, that is **1300M carried tokens down to 285M**. The model would have to read back **77%** of everything spilled before the win is gone.

What is left is no longer content, it is pointer: 3824 pointers averaging 305 B. Two thirds of the way there came from making the pointer cheap rather than from spilling more — a 150 B preview instead of 250 B (the *same* results spill, so it is free), and a path cut from 123 B to ~72 B by hashing the session id to 8 characters and capping the slug. The floor was never an independent number: it is whatever the pointer costs, which is why it could move from 800 B to 500 B only after the pointer moved.
